...
class Tokenizer:
	...
	def error(self, char):
		raise ValueError(
			"Unexpected character at %i:%i in %s: %s"
			% (self.row, self.col, self.state.__name__, char)
		)
...
class TokenStream:
	...
	def error(self, token):
		if token.type == TYPE_EOF:
			...
			message = "Unexpected token at %i:%i: %s" % (
				token.row,
				token.col,
				token.value,
			)
	...
	def read_reserved(self):
		return self.read_token(TYPE_RESERVED)

	def read_name(self):
		return self.read_token(TYPE_NAME)

	def read_symbol(self):
		return self.read_token(TYPE_SYMBOL)

	def parse_name(self):
		return self.parse_token(TYPE_NAME)

	def parse_number(self):
		return self.parse_token(TYPE_NUMBER)

	def parse_string(self):
		return self.parse_token(TYPE_STRING)

	def check_reserved(self, value):
		return self.check_token(TYPE_RESERVED, value)

	def check_symbol(self, value):
		return self.check_token(TYPE_SYMBOL, value)

	def check_eof(self):
		return self.check_token(TYPE_EOF, None)

	def skip_name(self, value):
		self.skip_token(TYPE_NAME, value)

	def skip_reserved(self, value):
		self.skip_token(TYPE_RESERVED, value)

	def skip_symbol(self, value):
		self.skip_token(TYPE_SYMBOL, value)
...
class Protocol:
	...
	def check(self):
		if self.noresponse and any(
			method.response.vars for method in self.methods.values()
		):
			raise ValueError(
				"%s is marked noresponse but at least one method returns a non-empty response"
				% self.name
			)
	...
	def add_method(self, method):
		...
		if method.id in self.methods:
			raise ValueError(
				"Method id %i is used twice in %s" % (method.id, self.name)
			)
...
STRING_TYPES = ["string", "buffer", "qbuffer", "stationurl"]
...
class Parser:
		...
		while True:
			...
			if token.type == TYPE_SYMBOL and token.value == "}":
				...
				if token.value == "method":
					protocol.add_method(self.parse_method(stream))
	...
	def parse_constant(self, stream, type):
		if type.name in NUMERIC_TYPES:
			return stream.parse_number()
		elif type.name in STRING_TYPES:
	...
	def parse_condition(self, stream, scope):
		...
		if token.value == "nex":
			cond.type = Condition.VERSION
		elif token.value == "revision":
			cond.type = Condition.REVISION
		else:
			stream.error(token)
...
EXTERNAL_TYPES = ["ResultRange", "NotificationEvent"]
...
class CodeGenerator:
	...
	def generate_struct(self, stream, struct):
		...
		if struct.parent:
			stream.write_line(
				'common.DataHolder.register(%s, "%s")' % (struct.name, struct.name)
			)
		stream.write_line()
	...
	def generate_struct_init_body(self, stream, body):
		for field in body.fields:
			if isinstance(field, Variable):
				stream.write_line(
					"self.%s = %s"
					% (field.name, self.make_constant(field.type, field.default))
				)
	...
	def generate_if_statement(self, stream, cond, prefix=""):
		if cond.type == Condition.VERSION:
			stream.write_line(
				'if %ssettings["nex.version"] >= %i:' % (prefix, cond.value)
			)
		else:
	...
	def generate_struct_check_body(self, stream, body):
		...
		for field in body.fields:
			if isinstance(field, Variable):
				if (
					field.type.name not in self.file.structs
					and field.type.name not in EXTERNAL_TYPES
	...
	def generate_struct_load_body(self, stream, body):
		...
		for field in body.fields:
			if isinstance(field, Variable):
				stream.write_line(
					"self.%s = %s" % (field.name, self.make_extract(field.type))
				)
			elif isinstance(field, Condition):
	...
	def generate_client_method(self, stream, proto, method):
		...
		if proto.noresponse:
			stream.write_line(
				"await self.client.request(self.PROTOCOL_ID, self.METHOD_%s, stream.get(), True)"
				% method.name.upper()
			)
		else:
			stream.write_line(
				"data = await self.client.request(self.PROTOCOL_ID, self.METHOD_%s, stream.get())"
				% method.name.upper()
			)
			...
			if len(method.response.vars) > 1:
				...
				for var in method.response.vars:
					stream.write_line(
						"obj.%s = %s" % (var.name, self.make_extract(var.type))
					)
			elif len(method.response.vars) == 1:
				value = method.response.vars[0]
				stream.write_line(
					"%s = %s" % (value.name, self.make_extract(value.type))
				)
			...
			stream.write_line(
				'logger.info("%s.%s -> done")' % (class_name, method.name)
			)
			if len(method.response.vars) > 1:
	...
	def generate_server(self, stream, proto):
		...
		for method in proto.methods.values():
			stream.write_line(
				"\tself.METHOD_%s: self.handle_%s," % (method.name.upper(), method.name)
			)
		stream.write_line("}")
		stream.unindent()
		...
		stream.write_line(
			'\t\tlogger.warning("Unknown method called on %s: %%i", method_id)'
			% server_name
		)
	...
	def generate_server_method(self, stream, proto, method):
		...
		stream.write_line(
			"async def handle_%s(self, client, input, output):" % method.name
		)

		if not method.supported:
			stream.write_line(
				'\tlogger.warning("%s.%s is not supported")' % (class_name, method.name)
			)
			stream.write_line('\traise common.RMCError("Core::NotImplemented")')
			return
		...
		for param in method.request.vars:
			stream.write_line(
				"%s = %s" % (param.name, self.make_extract(param.type, "input"))
			)
		...
		if len(method.response.vars) > 1:
			...
			for var in method.response.vars:
				stream.write_line(
					self.make_encode(var.type, "response.%s" % var.name, "output")
				)
		elif len(method.response.vars) == 1:
			...
			stream.write_line(
				'\traise RuntimeError("Expected %s, got %%s" %%response.__class__.__name__)'
				% expected
			)
	...
	def generate_server_stub(self, stream, proto, method):
		...
		stream.write_line(
			'\tlogger.warning("%s.%s not implemented")' % (class_name, method.name)
		)
		stream.write_line('\traise common.RMCError("Core::NotImplemented")')
	...
	def make_python_type(self, type):
		...
		if type.name == "variant":
			return "object"
		if type.name in ["buffer", "qbuffer"]:
			return "bytes"
		if type.name == "datetime":
			return "common.DateTime"
		if type.name == "stationurl":
			return "common.StationURL"
		if type.name == "result":
			return "common.Result"
		if type.name == "anydata":
			return "common.Data"
		if type.name == "ResultRange":
			return "common.ResultRange"
		if type.name == "NotificationEvent":
			return "notification.NotificationEvent"
		if type.name in self.file.structs:
			return type.name
		if type.name in NUMERIC_TYPES:
			return "int"
	...
	def make_constant(self, type, value):
		...
		if type.name == "datetime":
			return "common.DateTime(%i)" % value
		if type.name == "string":
			return '"%s"' % value
		if type.name == "stationurl":
			return 'common.StationURL.parse("%s")' % value
		if type.name in ["buffer", "qbuffer"]:
			return 'b"%s"' % value
		if type.name in NUMERIC_TYPES + ["bool"]:
			return str(value)
		if type.name == "list":
			entries = []
			for entry in value:
	...
	def make_extract(self, type, stream="stream"):
		...
		if type.name in MAPPED_TYPES:
			return "%s.%s()" % (stream, MAPPED_TYPES[type.name])
		if type.name in self.file.structs:
			return "%s.extract(%s)" % (stream, type.name)
		if type.name == "ResultRange":
			return "%s.extract(common.ResultRange)" % stream
		if type.name == "NotificationEvent":
			return "%s.extract(notification.NotificationEvent)" % stream
		if type.name == "list":
	...
	def make_extract_func(self, type, stream="stream"):
		...
		if type.name == "list":
			return "lambda: %s.list(%s)" % (
				stream,
				self.make_extract_func(type.template[0], stream),
			)
	...
	def make_encode_func(self, type, stream="stream"):
		...
		if type.name in self.file.structs:
			return "%s.add" % stream
		if type.name in EXTERNAL_TYPES:
			return "%s.add" % stream
		if type.name == "list":
			return "lambda x: %s.list(x, %s)" % (
				stream,
				self.make_encode_func(type.template[0], stream),
			)
		raise ValueError("Unknown type: %s" % type.name)
...
class DocsGenerator:
	...
	def generate_header(self, name):
		...
		self.text += (
			". This page was generated automatically from `%s.proto`.\n\n" % name
		)
		...
		for proto in self.file.protocols.values():
			if not proto.overridden:
				...
				self.text += (
					'<span class="docs">The client for the `%s`.</span>\n\n' % proto
				)
		...
		for proto in self.file.protocols.values():
			if not proto.overridden:
				...
				self.text += (
					'<span class="docs">The server for the `%s`.</span>\n\n' % proto
				)
	...
	def generate_client_method(self, method):
		param = ", ".join(
			[self.format_variable(param, False) for param in method.request.vars]
		)
	...
	def format_variable(self, var, defaults):
		...
		if defaults:
			if (
				...
				or var.type.name in ["ResultRange", "NotificationEvent"]
				or var.type.name in self.file.structs
			):
	...
	def format_type(self, type):
		...
		if type.name == "variant":
			return "object"
		if type.name == "datetime":
			return "[DateTime](common.md#datetime)"
		if type.name == "stationurl":
			return "[StationURL](common.md#stationurl)"
		if type.name == "result":
			return "[Result](common.md#result)"
		if type.name == "anydata":
			return "[Data](common.md)"
		if type.name == "ResultRange":
			return "[ResultRange](common.md#resultrange)"
		if type.name == "NotificationEvent":
			return "[NotificationEvent](notification.md#notificationevent)"
		if type.name in self.file.structs:
			return "[%s](#%s)" % (type.name, type.name.lower())
		if type.name in NUMERIC_TYPES:
			return "int"
		if type.name == "list":
			return "list[%s]" % self.format_type(type.template[0])
		if type.name == "map":
			return "dict[%s, %s]" % (
				self.format_type(type.template[0]),
				self.format_type(type.template[1]),
			)
		raise ValueError("Unknown type: %s" % type.name)
	...
	def format_constant(self, type, value):
		...
		if type.name in ["buffer", "qbuffer"]:
			return 'b"%s"' % value
		if type.name in ["string", "stationurl"]:
			return '"%s"' % value
		if type.name == "datetime":
			if value == 0:
