...
async def test_dauth_1200():
	...
	async with http.serve(handler, "localhost", 12345):
		keys = {
			"aes_kek_generation_source": bytes.fromhex(
...
async def test_dauth_1800():
	...
	async with http.serve(handler, "localhost", 12345):
		keys = {
			"aes_kek_generation_source": bytes.fromhex(
				"1092ce3d2c208c250ebe248537f2df73"
			),
...
async def test_dauth_2000():
	...
	async with http.serve(handler, "localhost", 12345):
		...
		client.set_context(None)
		await client.device_tokens([dauth.CLIENT_ID_BAAS, dauth.CLIENT_ID_PCTL])
...
async def test_edge_token_2000():
	...
	async with http.serve(handler, "localhost", 12345):
		...
		await client.edge_tokens(
			[(dauth.CLIENT_ID_BEACH, "akamai"), (dauth.CLIENT_ID_BCAT, "akamai")]
		)
