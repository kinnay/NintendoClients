...
class BAASClient:
	...
	async def authenticate(self, device_token, penne_id=None):
		...
		req.form = {"grantType": "public_client", "assertion": device_token}
	...
	async def login(
		...
		req.form = {"id": "%016x" % id, "password": password}
		...
		if self.system_version >= 1800:
			if na_country is None:
				raise ValueError(
					"na_country parameter is required for system version 18.0.0 and later"
				)
	...
	async def update_presence(
		...
		req = http.HTTPRequest.patch(
			"/1.0.0/users/%016x/device_accounts/%016x" % (user_id, device_account_id)
		)
		req.json = [
