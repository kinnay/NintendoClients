...
class DragonsClient:
	def __init__(self, device_id=None):
		...
		if self.device_id is not None:
			self.user_agent_nim = USER_AGENT % (
				common.FIRMWARE_VERSIONS[LATEST_VERSION],
				self.device_id,
			)
	...
	def set_system_version(self, version):
		...
		if self.device_id is not None:
			self.user_agent_nim = USER_AGENT % (
				common.FIRMWARE_VERSIONS[version],
				self.device_id,
			)
	...
	async def publish_elicense_archive(
		self, device_token, challenge, certificate, account_id
	):
		...
		response = await self.request(
			req, self.host_dragons, device_token, account_id=account_id
		)
		return response.json

	async def report_elicense_archive(
		self, device_token, elicense_archive_id, account_id
	):
		req = http.HTTPRequest.put(
			"/v%i/elicense_archives/%s/report" % (self.api_version, elicense_archive_id)
		)
	...
	async def publish_device_linked_elicenses(self, device_token):
		req = http.HTTPRequest.post(
			"/v%i/rights/publish_device_linked_elicenses" % self.api_version
		)
		response = await self.request(req, self.host_dragons, device_token)
		return response.json

	async def exercise_elicense(
		self, device_token, elicense_ids, account_ids, current_account_id
	):
		...
		await self.request(
			req, self.host_dragons, device_token, account_id=current_account_id
		)

	async def contents_authorization_token_for_aauth(
		self, device_token, elicense_id, na_id, title_id
	):
		if self.system_version < 1500:
			raise ValueError(
				"contents_authorization_token_for_aauth was added in system version 15.0.0"
			)

		req = http.HTTPRequest.post(
			"/v%i/contents_authorization_token_for_aauth/issue" % self.api_version
		)
		req.json = {"elicense_id": elicense_id, "na_id": "%016x" % na_id}
