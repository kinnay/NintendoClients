...
class DeviceAttribute:
	...
	def parse(cls, attribute):
		inst = cls()
		inst.created_date = datetime.datetime.fromisoformat(
...
class Email:
	...
	def parse(cls, email):
		...
		inst.validated_date = datetime.datetime.fromisoformat(
...
class Profile:
	...
	def parse(cls, profile):
		...
		inst.device_attributes = [
			DeviceAttribute.parse(attrib) for attrib in profile["device_attributes"]
		]
		if "forgot_pw_email_sent" in profile:
			inst.forgot_pw_email_sent = datetime.datetime.fromisoformat(
		...
		if "temporary_password_expiration" in profile:
			inst.temporary_password_expiration = datetime.datetime.fromisoformat(
...
class NNASClient:
	...
	def set_fpd_version(self, version):
		self.fpd_version = version
	...
	def prepare(self, req, auth=None, cert=None):
		...
		if self.title_id is not None:
			...
			req.headers["X-Nintendo-Unique-ID"] = "%05X" % (
				(self.title_id >> 8) & 0xFFFFF
			)
	...
	async def request(self, req):
		...
		if response.error():
			logger.error(
				"Account request returned status code %i\n%s",
	...
	async def get_pids(self, nnids):
		...
		response = await self.request(req)
		return {
			id["in_id"].text: int(id["out_id"].text)
			for id in response
			if id["out_id"].text
		}
	...
	async def get_nnids(self, pids):
		...
		response = await self.request(req)
		return {
			int(id["in_id"].text): id["out_id"].text
			for id in response
			if id["out_id"].text
		}

	async def get_mii(self, pid):
