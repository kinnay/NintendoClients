...
class AuthenticationClient(AuthenticationProtocol):
	...
	async def login(self, username):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_LOGIN, stream.get()
		)
	...
	async def login_ex(self, username, extra_data):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_LOGIN_EX, stream.get()
		)
	...
	async def request_ticket(self, source, target):
		...
		stream.pid(target)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_REQUEST_TICKET, stream.get()
		)
	...
	async def get_pid(self, username):
		...
		stream.string(username)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_PID, stream.get()
		)
	...
	async def login_with_context(self, login_data):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_LOGIN_WITH_CONTEXT, stream.get()
		)
...
class AuthenticationClientNX(AuthenticationProtocolNX):
	...
	async def validate_and_request_ticket(self, username):
		...
		stream.string(username)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_VALIDATE_AND_REQUEST_TICKET, stream.get()
		)
	...
	async def validate_and_request_ticket_with_param(self, param):
		...
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_VALIDATE_AND_REQUEST_TICKET_WITH_PARAM,
			stream.get(),
		)
		...
		logger.info(
			"AuthenticationClientNX.validate_and_request_ticket_with_param -> done"
		)
		return result
...
class AuthenticationServer(AuthenticationProtocol):
	...
	async def handle(self, client, method_id, input, output):
		if method_id in self.methods:
			...
			logger.warning(
				"Unknown method called on AuthenticationServer: %i", method_id
			)
...
class AuthenticationServerNX(AuthenticationProtocolNX):
	...
	async def handle(self, client, method_id, input, output):
		if method_id in self.methods:
			...
			logger.warning(
				"Unknown method called on AuthenticationServerNX: %i", method_id
			)
			raise common.RMCError("Core::NotImplemented")
	...
	async def handle_validate_and_request_ticket_with_custom_data(
		self, client, input, output
	):
		logger.info(
			"AuthenticationServerNX.validate_and_request_ticket_with_custom_data()"
		)
		# --- request ---
		username = input.string()
		extra_data = input.anydata()
		response = await self.validate_and_request_ticket_with_custom_data(
			client, username, extra_data
		)
	...
	async def handle_validate_and_request_ticket_with_param(
		self, client, input, output
	):
		...
		if not isinstance(response, ValidateAndRequestTicketResult):
			raise RuntimeError(
				"Expected ValidateAndRequestTicketResult, got %s"
	...
	async def validate_and_request_ticket(self, *args):
		logger.warning(
			"AuthenticationServerNX.validate_and_request_ticket not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def validate_and_request_ticket_with_custom_data(self, *args):
		logger.warning(
			"AuthenticationServerNX.validate_and_request_ticket_with_custom_data not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
	...
	async def validate_and_request_ticket_with_param(self, *args):
		logger.warning(
			"AuthenticationServerNX.validate_and_request_ticket_with_param not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
