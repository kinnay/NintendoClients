...
class HealthClient(HealthProtocol):
	...
	async def ping_daemon(self):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_PING_DAEMON, stream.get()
		)
	...
	async def ping_database(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_PING_DATABASE, stream.get()
		)
	...
	async def run_sanity_check(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_RUN_SANITY_CHECK, stream.get()
		)
	...
	async def fix_sanity_errors(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_FIX_SANITY_ERRORS, stream.get()
		)
