...
class AAUserClient(AAUserProtocol):
	...
	async def register_application(self, title_id):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_REGISTER_APPLICATION, stream.get()
		)
	...
	async def unregister_application(self, title_id):
		...
		stream.u64(title_id)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_UNREGISTER_APPLICATION, stream.get()
		)
	...
	async def set_application_info(self, application_info):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_SET_APPLICATION_INFO, stream.get()
		)
	...
	async def get_application_info(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_APPLICATION_INFO, stream.get()
		)
