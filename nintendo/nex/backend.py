...
class BackEndClient:
	...
	async def login(self, username, password=None, auth_info=None, servers=[]):
		...
		if not kerberos_key:
			...
			kerberos_key = self.key_derivation.derive_key(password.encode(), result.pid)

		# Decrypt ticket from login response
		ticket = kerberos.ClientTicket.decrypt(
			result.ticket, kerberos_key, self.settings
		)
		if ticket.target != secure_station["PID"]:
			...
			ticket = kerberos.ClientTicket.decrypt(
				response.ticket, kerberos_key, self.settings
			)
		...
		async with rmc.connect(
			self.settings, host, port, stream_id, context, creds, servers
		) as client:
			yield client
	...
	async def login_old(self, username, auth_info):
		...
		response.result.raise_if_error()
		return LoginResult(
			response.pid, response.ticket, None, response.connection_data.main_station
		)
