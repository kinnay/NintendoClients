...
class RankingClientMK8D(RankingProtocolMK8D):
	...
	async def get_cached_topx_ranking(self, category, order):
		...
		stream.add(order)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_CACHED_TOPX_RANKING, stream.get()
		)
	...
	async def get_cached_topx_rankings(self, categories, order):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_CACHED_TOPX_RANKINGS, stream.get()
		)
	...
	async def get_competition_ranking_score(self, param):
		...
		stream.add(param)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_COMPETITION_RANKING_SCORE, stream.get()
		)
	...
	async def upload_score_pack(self, score_data, metadata):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_UPLOAD_SCORE_PACK, stream.get()
		)
	...
	async def get_commmon_data_by_pid_list(self, pids):
		...
		stream.list(pids, stream.pid)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_COMMMON_DATA_BY_PID_LIST, stream.get()
		)
...
class RankingServerMK8D(RankingProtocolMK8D):
	...
	async def handle_get_cached_topx_ranking(self, client, input, output):
		...
		if not isinstance(response, RankingCachedResult):
			raise RuntimeError(
				"Expected RankingCachedResult, got %s" % response.__class__.__name__
			)
		output.add(response)
	...
	async def handle_get_commmon_data_by_pid_list(self, client, input, output):
		...
		if not isinstance(response, CommonDataList):
			raise RuntimeError(
				"Expected CommonDataList, got %s" % response.__class__.__name__
			)
		output.add(response)
	...
	async def get_ranking_by_unique_id_list(self, *args):
		logger.warning(
			"RankingServerMK8D.get_ranking_by_unique_id_list not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
	...
	async def get_competition_ranking_score(self, *args):
		logger.warning(
			"RankingServerMK8D.get_competition_ranking_score not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def upload_competition_ranking_score(self, *args):
		logger.warning(
			"RankingServerMK8D.upload_competition_ranking_score not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
