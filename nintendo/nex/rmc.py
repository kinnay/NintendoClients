...
class RMCClient:
	...
	def register_server(self, server):
		if server.PROTOCOL_ID in self.servers:
			raise ValueError(
				"Server with protocol id %i already exists" % server.PROTOCOL_ID
			)
	...
	async def handle_request(self, request):
		...
		if request.protocol in self.servers:
			try:
				await self.servers[request.protocol].handle(
					self, request.method, input, output
				)
			...
			logger.warning(
				"Received RMC request with unimplemented protocol id: %i",
				request.protocol,
			)
