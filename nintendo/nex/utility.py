...
class UtilityClient(UtilityProtocol):
	...
	async def acquire_nex_unique_id(self):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_ACQUIRE_NEX_UNIQUE_ID, stream.get()
		)
	...
	async def acquire_nex_unique_id_with_password(self):
		...
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_ACQUIRE_NEX_UNIQUE_ID_WITH_PASSWORD,
			stream.get(),
		)
	...
	async def associate_nex_unique_id_with_my_principal_id(self, info):
		...
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_ASSOCIATE_NEX_UNIQUE_ID_WITH_MY_PRINCIPAL_ID,
			stream.get(),
		)
		...
		logger.info(
			"UtilityClient.associate_nex_unique_id_with_my_principal_id -> done"
		)
	...
	async def associate_nex_unique_ids_with_my_principal_id(self, infos):
		...
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_ASSOCIATE_NEX_UNIQUE_IDS_WITH_MY_PRINCIPAL_ID,
			stream.get(),
		)
		...
		logger.info(
			"UtilityClient.associate_nex_unique_ids_with_my_principal_id -> done"
		)
	...
	async def get_associated_nex_unique_id_with_my_principal_id(self):
		...
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_GET_ASSOCIATED_NEX_UNIQUE_ID_WITH_MY_PRINCIPAL_ID,
			stream.get(),
		)
		...
		logger.info(
			"UtilityClient.get_associated_nex_unique_id_with_my_principal_id -> done"
		)
		return info

	async def get_associated_nex_unique_ids_with_my_principal_id(self):
		logger.info(
			"UtilityClient.get_associated_nex_unique_ids_with_my_principal_id()"
		)
		# --- request ---
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_GET_ASSOCIATED_NEX_UNIQUE_IDS_WITH_MY_PRINCIPAL_ID,
			stream.get(),
		)
		...
		logger.info(
			"UtilityClient.get_associated_nex_unique_ids_with_my_principal_id -> done"
		)
		return infos
	...
	async def get_integer_settings(self, index):
		...
		stream.u32(index)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_INTEGER_SETTINGS, stream.get()
		)
	...
	async def get_string_settings(self, index):
		...
		stream.u32(index)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_STRING_SETTINGS, stream.get()
		)
...
class UtilityServer(UtilityProtocol):
	...
	async def handle_acquire_nex_unique_id_with_password(self, client, input, output):
		...
		if not isinstance(response, UniqueIdInfo):
			raise RuntimeError(
				"Expected UniqueIdInfo, got %s" % response.__class__.__name__
			)
		output.add(response)

	async def handle_associate_nex_unique_id_with_my_principal_id(
	...
	async def handle_associate_nex_unique_ids_with_my_principal_id(
		self, client, input, output
	):
	...
	async def handle_get_associated_nex_unique_id_with_my_principal_id(
		self, client, input, output
	):
	...
	async def handle_get_associated_nex_unique_ids_with_my_principal_id(
		self, client, input, output
	):
		logger.info(
			"UtilityServer.get_associated_nex_unique_ids_with_my_principal_id()"
		)
		# --- request ---
	...
	async def acquire_nex_unique_id_with_password(self, *args):
		logger.warning(
			"UtilityServer.acquire_nex_unique_id_with_password not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def associate_nex_unique_id_with_my_principal_id(self, *args):
		logger.warning(
			"UtilityServer.associate_nex_unique_id_with_my_principal_id not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def associate_nex_unique_ids_with_my_principal_id(self, *args):
		logger.warning(
			"UtilityServer.associate_nex_unique_ids_with_my_principal_id not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def get_associated_nex_unique_id_with_my_principal_id(self, *args):
		logger.warning(
			"UtilityServer.get_associated_nex_unique_id_with_my_principal_id not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def get_associated_nex_unique_ids_with_my_principal_id(self, *args):
		logger.warning(
			"UtilityServer.get_associated_nex_unique_ids_with_my_principal_id not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
