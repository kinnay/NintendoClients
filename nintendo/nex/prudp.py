	...
	def decode(self, data):
		...
		while not stream.eof():
			...
			if checksum != expected_checksum:
				raise ValueError(
					"(V0) Invalid checksum (expected %i, got %i)"
...
class PRUDPMessageV1:
	...
	def encode_options(self, packet):
		options = {}
		if packet.type in [TYPE_SYN, TYPE_CONNECT]:
			options[OPTION_SUPPORT] = packet.minor_version | (
				packet.supported_functions << 8
			)
	...
	def verify_options(self, packet, options):
		...
		if packet.type == TYPE_SYN:
			return keys == {
				OPTION_SUPPORT,
				OPTION_CONNECTION_SIG,
				OPTION_MAX_SUBSTREAM_ID,
			}
		if packet.type == TYPE_CONNECT:
			return keys == {
				OPTION_SUPPORT,
				OPTION_CONNECTION_SIG,
				OPTION_UNRELIABLE_SEQ_ID,
				OPTION_MAX_SUBSTREAM_ID,
			}
		if packet.type == TYPE_DATA:
...
class PRUDPMessageSelector:
	...
	def calc_packet_signature(self, packet, session_key, connection_signature):
		return self.select(packet.version).calc_packet_signature(
...
class ZlibCompression:
	...
	def decompress(self, data):
		...
		if ratio != data[0]:
			raise ValueError(
				"Unexpected compression ratio (expected %i, got %i)" % ratio, data[0]
			)
		return decompressed
...
class PayloadEncoder:
	...
	def __init__(self, settings):
		...
		self.reliable_encryption = [
			self.create_encryption(settings) for i in range(substreams)
		]
...
class PRUDPClient:
	...
	async def send_connect(self):
		connection_signature = self.packet_encoder.calc_connection_signature(
			self.remote_addr
		)
		packet = PRUDPPacket(
	...
	async def send_packet(self, packet):
		...
		if packet.type == TYPE_SYN:
			packet.signature = self.packet_encoder.calc_packet_signature(
				packet, b"", b""
			)
		elif packet.type == TYPE_CONNECT:
			packet.signature = self.packet_encoder.calc_packet_signature(
				packet, b"", self.remote_signature
			)
		else:
			packet.signature = self.packet_encoder.calc_packet_signature(
				packet, self.session_key, self.remote_signature
			)
		...
		if (
			packet.flags & FLAG_RELIABLE or packet.type == TYPE_SYN
		) and packet.flags & FLAG_NEED_ACK:
	...
	async def resend_packet(self, packet, counter):
		...
		if counter < self.resend_limit:
			...
			handle = self.scheduler.schedule(
				self.resend_packet, self.resend_timeout, packet, counter + 1
			)
	...
	def schedule_timeout(self, packet):
		key = (packet.type, packet.substream_id, packet.packet_id)
		handle = self.scheduler.schedule(
			self.resend_packet, self.resend_timeout, packet, 0
		)
		self.ack_events[key] = handle
	...
	async def process_syn(self, packet):
		if packet.signature != self.packet_encoder.calc_packet_signature(
			packet, b"", b""
		):
		...
		if (
			packet.session_id != 0
			or packet.packet_id != 0
			or packet.fragment_id != 0
			or packet.substream_id != 0
		):
			raise ValueError("Received invalid SYN/ACK packet")
		if (
			packet.max_substream_id > self.max_substream_id
			or packet.minor_version > self.minor_ver
			or packet.supported_functions & ~self.supported_functions
		):
			raise ValueError(
				"Received SYN/ACK packet with invalid negotiation parameters"
			)
	...
	async def process_connect(self, packet):
		...
		if (
			...
			or packet.fragment_id != 0
			or packet.substream_id != 0
			or any(packet.connection_signature)
		):
		...
		if (
			...
			raise ValueError(
				"Received CONNECT/ACK packet with invalid negotiation parameters"
			)
	...
	async def process_other(self, packet):
		...
		if packet.signature != self.packet_encoder.calc_packet_signature(
			packet, self.session_key, connection_signature
		):
		...
		if packet.flags & FLAG_MULTI_ACK:
			...
			if packet.substream_id > self.max_substream_id:
				raise ValueError(
					"Received packet with invalid substream id: %i", packet.substream_id
				)
	...
	def verify_aggregate_ack(self, packet):
		...
		if packet.substream_id not in [0, 1]:
			raise ValueError(
				"Aggregate ack packet has invalid stream id: %i" % packet.substream_id
			)
	...
	def handle_aggregate_ack(self, packet):
		...
		if self.is_new_aggregate_ack(packet):
			...
			extra_ids = struct.unpack_from(
				"<%iH" % packet.payload[1], packet.payload, 4
			)
		else:
			substream = 0
			base_id = packet.packet_id
			extra_ids = struct.unpack(
				"<%iH" % (len(packet.payload) // 2), packet.payload
			)
...
class PRUDPServerStream:
	...
	async def process_syn(self, packet, addr):
		...
		ack.connection_signature = self.packet_encoder.calc_connection_signature(
			addr, ack.version
		)
	...
	async def process_connect(self, packet, addr):
		...
		if (
			not packet.flags & FLAG_NEED_ACK
			or packet.packet_id != 1
			or packet.fragment_id != 0
			or packet.substream_id != 0
		):
		...
		if (
			packet.max_substream_id > self.max_substream_id
			or packet.minor_version > self.minor_ver
			or packet.supported_functions & ~self.supported_functions != 0
		):
			raise ValueError(
				"Received CONNECT packet with invalid negotiation parameters"
			)
		...
		if client is None:
			...
			client.configure(
				packet.max_substream_id,
				packet.supported_functions,
		...
		ack.signature = self.packet_encoder.calc_packet_signature(
			ack, b"", packet.connection_signature
		)
		await self.transport.send(ack, addr)
...
class PRUDPClientTransport:
	def __init__(self, settings, socket, group):
		...
		self.packet_encoder = PRUDPMessageSelector(settings).select(
			settings["prudp.version"]
		)

	@contextlib.asynccontextmanager
	async def connect(
		self, port, type=10, credentials=None, *, disconnect_timeout=None
	):
	...
	async def process_data(self, data):
		try:
			...
			logger.warning(
				"[CLI] An exception occurred while processing a packet: %s" % e
			)
...
class PRUDPServerTransport:
	...
	async def serve(self, handler, port, type=10, key=None, *, disconnect_timeout=None):
		async with util.create_task_group() as group:
			stream = PRUDPServerStream(
				self.settings, self, handler, key, group, disconnect_timeout
			)
	...
	async def process_data(self, data, addr):
		try:
			...
			logger.warning(
				"[SRV] An exception occurred while processing a packet: %s" % e
			)
...
def connect_transport_socket(settings, host, port, context):
	...
	return websocket.connect(
		"%s:%i" % (host, port), context, protocols=["NEX"], disconnect_timeout=0
	)
...
def serve_transport_socket(handler, settings, host, port, context):
	...
	return websocket.serve(
		handler, host, port, context, protocol="NEX", disconnect_timeout=0
	)
...
async def serve_transport(settings, host="", port=0, context=None):
	...
	if transport == settings.TRANSPORT_UDP:
		...
		async with serve_transport_socket(
			transport.handle, settings, host, port, context
		):
			yield transport
	...
	*,
	...
	async with connect_transport(settings, host, port, context) as transport:
		async with transport.connect(
			vport, type, credentials, disconnect_timeout=disconnect_timeout
		) as client:
			yield client
	...
	*,
	...
	async with serve_transport(settings, host, port, context) as transport:
		async with transport.serve(
			handler, vport, type, key, disconnect_timeout=disconnect_timeout
		):
			yield
