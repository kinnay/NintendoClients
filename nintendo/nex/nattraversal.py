...
class NATTraversalClient(NATTraversalProtocol):
	...
	async def request_probe_initiation(self, target_urls):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_REQUEST_PROBE_INITIATION, stream.get()
		)
	...
	async def initiate_probe(self, station_to_probe):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_INITIATE_PROBE, stream.get()
		)
	...
	async def request_probe_initiation_ext(self, target_urls, station_to_probe):
		...
		stream.stationurl(station_to_probe)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_REQUEST_PROBE_INITIATION_EXT, stream.get()
		)
	...
	async def report_nat_traversal_result(self, cid, result):
		...
		stream.bool(result)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_REPORT_NAT_TRAVERSAL_RESULT, stream.get()
		)
	...
	async def report_nat_properties(self, natm, natf, rtt):
		...
		stream.u32(rtt)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_REPORT_NAT_PROPERTIES, stream.get()
		)
	...
	async def get_relay_signature_key(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_RELAY_SIGNATURE_KEY, stream.get()
		)
	...
	async def report_nat_traversal_result_detail(self, cid, result, detail, rtt):
		...
		data = await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_REPORT_NAT_TRAVERSAL_RESULT_DETAIL,
			stream.get(),
		)
...
class NATTraversalServer(NATTraversalProtocol):
	...
	async def request_probe_initiation_ext(self, *args):
		logger.warning(
			"NATTraversalServer.request_probe_initiation_ext not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
	...
	async def report_nat_traversal_result_detail(self, *args):
		logger.warning(
			"NATTraversalServer.report_nat_traversal_result_detail not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
