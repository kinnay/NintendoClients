...
common.DataHolder.register(
	NintendoNotificationEventGeneral, "NintendoNotificationEventGeneral"
)
...
common.DataHolder.register(
	NintendoNotificationEventKeyValue, "NintendoNotificationEventKeyValue"
)
...
common.DataHolder.register(
	NintendoNotificationEventProfile, "NintendoNotificationEventProfile"
)
...
class NintendoNotificationClient(NintendoNotificationProtocol):
	...
	async def process_nintendo_notification_event(self, event):
		...
		await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_PROCESS_NINTENDO_NOTIFICATION_EVENT,
			stream.get(),
			True,
		)
	...
	async def process_nintendo_notification_event_alt(self, event):
		logger.info(
			"NintendoNotificationClient.process_nintendo_notification_event_alt()"
		)
		# --- request ---
		stream = streams.StreamOut(self.settings)
		stream.add(event)
		await self.client.request(
			self.PROTOCOL_ID,
			self.METHOD_PROCESS_NINTENDO_NOTIFICATION_EVENT_ALT,
			stream.get(),
			True,
		)
...
class NintendoNotificationServer(NintendoNotificationProtocol):
	...
	async def handle(self, client, method_id, input, output):
		if method_id in self.methods:
			...
			logger.warning(
				"Unknown method called on NintendoNotificationServer: %i", method_id
			)
	...
	async def handle_process_nintendo_notification_event_alt(
		self, client, input, output
	):
		logger.info(
			"NintendoNotificationServer.process_nintendo_notification_event_alt()"
		)
		# --- request ---
		event = input.extract(NintendoNotificationEvent)
	...
	async def process_nintendo_notification_event(self, *args):
		logger.warning(
			"NintendoNotificationServer.process_nintendo_notification_event not implemented"
		)
		raise common.RMCError("Core::NotImplemented")

	async def process_nintendo_notification_event_alt(self, *args):
		logger.warning(
			"NintendoNotificationServer.process_nintendo_notification_event_alt not implemented"
		)
		raise common.RMCError("Core::NotImplemented")
