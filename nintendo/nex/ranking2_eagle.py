...
class Ranking2ChartInfo(common.Structure):
	...
	def check_required(self, settings, version):
		for field in [
			...
			"score_order",
			"estimate_length",
			"estimate_highest_score",
			"estimate_lowest_score",
			"estimate_median_score",
			"estimate_average_score",
			"highest_bins_score",
			"lowest_bins_score",
			"bins_width",
			"attribute1",
...
class Ranking2Client(Ranking2Protocol):
	...
	async def put_score(self, socres, unique_id):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_PUT_SCORE, stream.get()
		)
	...
	async def put_common_data(self, data, unique_id):
		...
		stream.u64(unique_id)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_PUT_COMMON_DATA, stream.get()
		)
	...
	async def get_ranking_by_principal_id(self, param, pids):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_RANKING_BY_PRINCIPAL_ID, stream.get()
		)
	...
	async def get_category_setting(self, category):
		...
		stream.u32(category)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_CATEGORY_SETTING, stream.get()
		)
	...
	async def get_ranking_chart(self, input):
		...
		stream.add(input)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_RANKING_CHART, stream.get()
		)
	...
	async def get_ranking_charts(self, inputs):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_RANKING_CHARTS, stream.get()
		)
	...
	async def get_estimate_score_rank(self, input):
		...
		stream.add(input)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_ESTIMATE_SCORE_RANK, stream.get()
		)
	...
	async def get_estimate_my_score_rank(self, input):
		...
		stream.add(input)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_ESTIMATE_MY_SCORE_RANK, stream.get()
		)
...
class Ranking2Server(Ranking2Protocol):
	...
	async def handle_get_common_data(self, client, input, output):
		...
		if not isinstance(response, Ranking2CommonData):
			raise RuntimeError(
				"Expected Ranking2CommonData, got %s" % response.__class__.__name__
			)
		output.add(response)
	...
	async def handle_get_ranking(self, client, input, output):
		...
		if not isinstance(response, Ranking2Info):
			raise RuntimeError(
				"Expected Ranking2Info, got %s" % response.__class__.__name__
			)
		output.add(response)
	...
	async def handle_get_category_setting(self, client, input, output):
		...
		if not isinstance(response, Ranking2CategorySetting):
			raise RuntimeError(
				"Expected Ranking2CategorySetting, got %s" % response.__class__.__name__
			)
		output.add(response)
	...
	async def handle_get_ranking_chart(self, client, input, output):
		...
		if not isinstance(response, Ranking2ChartInfo):
			raise RuntimeError(
				"Expected Ranking2ChartInfo, got %s" % response.__class__.__name__
			)
		output.add(response)
	...
	async def handle_get_estimate_score_rank(self, client, input, output):
		...
		if not isinstance(response, Ranking2EstimateScoreRankOutput):
			raise RuntimeError(
				"Expected Ranking2EstimateScoreRankOutput, got %s"
