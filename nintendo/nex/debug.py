...
class DebugClient(DebugProtocol):
	...
	async def enable_api_recorder(self):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_ENABLE_API_RECORDER, stream.get()
		)
	...
	async def disable_api_recorder(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_DISABLE_API_RECORDER, stream.get()
		)
	...
	async def is_api_recorder_enabled(self):
		...
		stream = streams.StreamOut(self.settings)
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_IS_API_RECORDER_ENABLED, stream.get()
		)
	...
	async def get_api_calls(self, pids, unk1, unk2):
		...
		data = await self.client.request(
			self.PROTOCOL_ID, self.METHOD_GET_API_CALLS, stream.get()
		)
...
class DebugServer(DebugProtocol):
	...
	async def handle_set_exclude_joined_matchmake_session(self, client, input, output):
		logger.warning(
			"DebugServer.set_exclude_joined_matchmake_session is not supported"
		)
		raise common.RMCError("Core::NotImplemented")

	async def handle_get_exclude_joined_matchmake_session(self, client, input, output):
		logger.warning(
			"DebugServer.get_exclude_joined_matchmake_session is not supported"
		)
		raise common.RMCError("Core::NotImplemented")
