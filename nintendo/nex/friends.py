...
class FriendsClient:
	...
	def get_all_information(self, nna_info, presence, birthday):
		...
		friends = stream.list(lambda: stream.extract(FriendInfo))
		sent_requests = stream.list(lambda: stream.extract(FriendRequest))
		received_requests = stream.list(lambda: stream.extract(FriendRequest))
		blacklist = stream.list(lambda: stream.extract(BlacklistedPrincipal))
		unk1 = stream.bool()
		notifications = stream.list(lambda: stream.extract(PersistentNotification))
		unk2 = stream.u8()
		logger.info("Friends.get_all_information -> ...")
