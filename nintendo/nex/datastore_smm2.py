...
class DataStoreClientSMM2(DataStoreProtocolSMM2):
	...
	async def prepare_get_object_or_meta(self, param):
		logger.info("DataStoreClientSMM2.prepare_get_object_or_meta()")
		#--- request ---
		stream = streams.StreamOut(self.settings)
		stream.add(param)
		...
		if not stream.eof():
			raise ValueError("Response is bigger than expected (got %i bytes, but only %i were read)" %(stream.size(), stream.tell()))
		logger.info("DataStoreClientSMM2.prepare_get_object_or_meta -> done")
		return obj
...
class DataStoreServerSMM2(DataStoreProtocolSMM2):
	...
	async def handle_prepare_get_object_or_meta(self, client, input, output):
		logger.info("DataStoreServerSMM2.prepare_get_object_or_meta()")
		#--- request ---
		param = input.extract(DataStorePrepareGetParam)
	...
	async def prepare_get_object_or_meta(self, *args):
		logger.warning("DataStoreServerSMM2.prepare_get_object_or_meta not implemented")
		raise common.RMCError("Core::NotImplemented")
